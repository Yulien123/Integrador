...
class PersonasController {
    ...
    async get(req, res, next) {
        ...
        try {
            ...
            const personasConFechaFormateada = personas.map(persona => {
                const fechaFormateada = obtenerFechaFormateada(new Date(persona.nacimiento));
                return { ...persona, nacimiento: fechaFormateada };
            });

            const { nombreUpdate, nombreStore, nombreDelete } = req.query; // Obtener 'nombre' y 'nombreDelete' de los parámetros de consulta

            // Determinar el mensaje a mostrar basado en los parámetros de consulta
            let mensaje = null;
            if (nombreDelete) {
                mensaje = `Se ha Eliminado una persona`;
            ...
            // Renderizar la vista con el mensaje adecuado
            res.render('personas/index', { personas: personasConFechaFormateada, mensaje });
        } catch (error) {
            console.error('Error al obtener personas desde el controlador:', error);
            next(error);
        }
    }
